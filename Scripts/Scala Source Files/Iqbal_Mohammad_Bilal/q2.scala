...
val precount= during_game.map (x=> ((x._2, x._7), 1)).reduceByKey((accum, n) => (accum + n)).map (x=> (x._1._1, 1)).reduceByKey((accum, n) => (accum + n))
val precount2= during_game2.map (x=> ((x._2, x._7), 1)).reduceByKey((accum, n) => (accum + n)).map (x=> (x._1._1, 1)).reduceByKey((accum, n) => (accum + n))

val almostfinal= precount.join(precount2)
val afinal= almostfinal.map(x=> (x._1, (x._2._1+x._2._2)))

val asd= afinal.sortBy(_._2, false).zipWithIndex.filter(_._2<10)
asd.saveAsTextFile("/user/training/q2.txt")
