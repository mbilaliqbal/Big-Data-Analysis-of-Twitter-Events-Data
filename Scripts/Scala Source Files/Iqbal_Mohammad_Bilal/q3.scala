...
//Counting total games played for each team 
val gameteamscount1= game3.map (x=> (x(0), x(1), x(2))).map (x=>(x._2, 1)).reduceByKey((accum, n) => (accum + n))
val gameteamscount2= game3.map (x=> (x(0), x(1), x(2))).map (x=>(x._3, 1)).reduceByKey((accum, n) => (accum + n))
val almostcount= gameteamscount1.join(gameteamscount2)
val totalgames= almostcount.map(x=> (x._1, x._2._1+x._2._2))
...
//counting tweets per team
val precount= during_game.map(x=> (x._5, (x._1))).groupByKey().map(x=>(x._1, x._2.mkString)).map(x=>(x._2, 1)).reduceByKey((accum, n) => (accum + n))
val precount2= during_game2.map(x=> (x._5, (x._1))).groupByKey().map(x=>(x._1, x._2.mkString)).map(x=>(x._2, 1)).reduceByKey((accum, n) => (accum + n))
val totalcount= precount.join(precount2).map(x=> (x._1, (x._2._1+x._2._2)))

//join with total games RDD and get average
...
asd.saveAsTextFile("/user/training/q3.txt")
